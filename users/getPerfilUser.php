...
try {
    ...
    $query = "SELECT `id_usuario`,`nombre_usuario`,`email_user`,`telefono_user` FROM `usuarios` WHERE id_usuario = :id";
    $result = $pdo->prepare($query);
    $result->bindValue(':id', $id_usuario);
    $result->execute();
foreach($result as $row){
    $data['data'] = ['id_usuario' => $row['id_usuario'],'nombre_usuario'=>$row['nombre_usuario'],'email_user'=>$row['email_user'], 'telefono_user'=>$row['telefono_user']   ];
}
